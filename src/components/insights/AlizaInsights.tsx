...
export default function AlizaInsights({ userId }: AlizaInsightsProps) {
  ...
  const loadInsights = async () => {
    setIsLoading(true);
    try {
      console.log('🔍 Loading insights from database for user:', userId);
      ...
      console.log('🔍 Loading all insights for user:', userId);
      ...
      console.log('📊 All insights loaded:', {
      ...
      if (insightsError) {
        console.error('❌ Error loading insights from database:', insightsError);
        console.error('❌ Error details:', JSON.stringify(insightsError, null, 2));
        setInsights([]);
        setHeroInsight(null);
        setIsLoading(false);
        return;
      ...
      if (!insightsData || insightsData.length === 0) {
        ...
        setHasData(hasAnyData);
        if (hasAnyData) {
          console.log('✅ User has data, will show generate button');
        } else {
          console.log('⚠️ No data available for user');
        }
      }

      if (!insightsData || insightsData.length === 0) {
        console.log('⚠️ No insights found in database. Insights will be generated by the daily cron job or can be generated manually.');
        setInsights([]);
        setHeroInsight(null);
        setIsLoading(false);
        return;
      ...
      console.log('📋 Insights details:', loadedInsights.map(i => ({ 
        id: i.id, 
        title: i.title, 
        priority: i.priority,
      ...
      // אבל נבחר את התובנה החדשה ביותר (האחרונה) כ-Hero, לא את הראשונה
      ...
      console.log('🎯 Selected Hero insight:', hero ? { id: hero.id, title: hero.title } : 'none');
      
      setHeroInsight(hero || null);
      // נציג את כל התובנות, כולל את ה-Hero (הוא יוצג גם בנפרד כ-Hero וגם ברשימה)
      // כך המשתמשת תראה את כל התובנות שנוצרו
      console.log('📊 Setting insights state:', {
        hero: hero ? hero.id : null,
        totalInsights: loadedInsights.length,
        allInsightsIncluded: true
      });
      // נציג את כל התובנות, לא רק את השאר
      ...
      <div className="insights-section">
        ...
        <div className="insights-grid">
          {insights.length === 0 && !heroInsight ? (
                ...
                <div className="generate-insights-action">
                  <button 
                    ...
                    onClick={async () => {
                      setIsLoading(true);
                      try {
                        console.log('🔄 Generating insights manually...');
                        ...
                        if (result.deduct_tokens && result.deduct_tokens > 0) {
                          console.log('🔄 Updating tokens after insights generation:', result.deduct_tokens);
                        ...
                        console.log('🔄 Reloading insights from database...');
                        await loadInsights();
                        console.log('✅ Insights reloaded');
                      } catch (error) {
            ...
            // הצג את כל התובנות, מסודרות לפי תאריך (החדשות ביותר קודם)
            insights
              .sort((a, b) => {
                // מצא את התאריך של כל תובנה מהנתונים המקוריים
              ...
              .map((insight) => {
                // מצא את התאריך האמיתי של התובנה מהנתונים המקוריים
                ...
                <div className="actionable-section">
                  {selectedInsight.actionableSteps.reliefMethods && selectedInsight.actionableSteps.reliefMethods.length > 0 && (
                    <div className="actionable-subsection">
                      <h4>
                        <span>💡</span>
                        <span>דרכים להקל על התסמינים:</span>
                      </h4>
                      <ul>
